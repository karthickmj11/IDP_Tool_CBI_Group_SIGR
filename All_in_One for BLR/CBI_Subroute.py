...
def run(loc,op_l):
    ...
    sh = wb['Subroute']
    f = open(op_l+'\\Subroute.xml','w')
    f.write('''<?xml version="1.0" ?>
<ObjectPropertyModule Generation_Date="2019-02-25" Project="BLR" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="SubRoute.xsd">
  <Versions>
...
\n''')
    f.write('  <Classes>\n')
    f.write('    <Class name="SubRoute">\n')
    f.write('      <Objects>\n')
    i=2
    sector = str(sh.cell(row=2,column=2).value).split('_')[0][-1:]
    while(sh.cell(row=i,column=2).value!=None):
        f.write('        <Object name="'+str(sh.cell(row=i,column=2).value)+'" rules="update_or_create">\n')
        f.write('          <Properties>\n')
        f.write('            <Property dt="boolean" name="HILCAvailable">false</Property>\n')
        ...
        f.write('            <Property dt="string" name="Status_PV">U'+str(sh.cell(row=i,column=2).value)[2:]+'</Property>\n')
