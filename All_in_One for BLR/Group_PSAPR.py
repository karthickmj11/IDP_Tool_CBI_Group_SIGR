...
<ObjectPropertyModule Generation_Date="2019-02-25" Project="BLR" name="AreaFunctionGroup" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="OPFormat.xsd">
  <Versions>
    ...
    f.write('<Classes>\n')
    f.write('<Class name="PSAPR" rules="update" traces="error">\n')
    f.write('<Objects>\n')
    while(sh.cell(row=i,column=2).value!=None):
        ...
        f.write('            <Property dt="string" name="FunctionGroup">Function/Signalling/PSAPR/'+str(sh.cell(row=i,column=2).value)+'</Property>\n')
        f.write('          </Properties>\n')
        f.write('        </Object>\n')
        i+=1
    f.write('      </Objects>\n')
