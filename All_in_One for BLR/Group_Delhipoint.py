...
def run(loc,op_l):
    ...
    f = open(op_l+'\\Group_BLR_Point.xml','w')
    sh = wb['Switch']
    sh1 = wb['Interlocking']
    i=2
...
<ObjectPropertyModule Generation_Date="2019-02-25" Project="BLR" name="AreaFunctionGroup" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="OPFormat.xsd">
  <Versions>
    ...
    f.write('  <Classes>\n')
    f.write('    <Class name="BLR_Point" rules="update" traces="error">\n')
    f.write('      <Objects>\n')
    while(sh.cell(row=i,column=1).value!=None):
        f.write('        <Object name="SWB'+str(sh.cell(row=i,column=2).value)[2:]+'" rules="update" traces="error">\n')
        f.write('          <Properties>\n')
        f.write('            <Property dt="string" name="AreaGroup">Area/LI_1/MI_1/Territory_'+str(u)+'/Area_'+str(u)+'</Property>\n')
        f.write('            <Property dt="string" name="FunctionGroup">Function/Signalling/Switch/SWB'+str(sh.cell(row=i,column=2).value)[2:]+'</Property>\n')
        f.write('          </Properties>\n')
        f.write('        </Object>\n')
        i+=1
    f.write('      </Objects>\n')
