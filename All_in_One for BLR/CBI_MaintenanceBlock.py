...
def run(loc,op_l):
    ...
    f = open(op_l+'\\MaintenanceBlock.xml','w')
    sh = wb['MBL']
    i=2
    f.write('''<?xml version="1.0" ?>
<ObjectPropertyModule Generation_Date="2019-02-25" Project="BLR" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="MaintenanceBlock.xsd">
  <Versions>
    ...
    f.write('<Classes>\n')
    f.write('<Class name="MaintenanceBlock">\n')
    f.write('<Objects>\n')
    while(sh.cell(row=i,column=2).value!=None):
        f.write('        <Object name="'+str(sh.cell(row=i,column=2).value).strip()+'" rules="update_or_create">\n')
        f.write('          <Properties>\n')
