...
def run(loc,op_l):
    ...
    f = open(op_l+'\\Cycle.xml','w')
    sh = wb['Cycle']
    sh1 = wb['Interlocking']
    i=2
    u = sh1.cell(row=1,column=1).value
    f.write('''<?xml version="1.0" ?>
<ObjectPropertyModule Generation_Date="2019-02-25" Project="BLR" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="Cycle.xsd">
  <Versions>
    ...
    f.write('<Classes>\n')
    f.write('<Class name="Cycle">\n')
    f.write('<Objects>\n')
    while(sh.cell(row=i,column=2).value!=None):
        f.write('        <Object name="'+str(sh.cell(row=i,column=2).value)+'" rules="update_or_create">\n')
        f.write('          <Properties>\n')
        f.write('            <Property dt="boolean" name="isRouteSequence">0</Property> \n')
        f.write('            <Property dt="string" name="ManagedKeys">CY_CYC_RES;CY_CYD_RES;CY_NOTSET;CY_SET</Property>\n')
        f.write('            <MultiLingualProperty name="Name">\n')
        f.write('               <MultiLingualValue localeId="1033" roleId="-1">CY'+str(sh.cell(row=i,column=2).value)[1:]+'</MultiLingualValue>\n')
        f.write('               <MultiLingualValue localeId="1036" roleId="-1">CY'+str(sh.cell(row=i,column=2).value)[1:]+'</MultiLingualValue>\n')
