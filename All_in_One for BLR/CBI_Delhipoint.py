...
def run(loc,op_l):
    ...
    f = open(op_l+'\\BLR_Point.xml','w')
    sh = wb['Switch']
    i=2
    sector = str(sh.cell(row=1,column=2).value)[-1:]
    f.write('''<?xml version="1.0" ?>
<ObjectPropertyModule Generation_Date="2019-02-25" Project="BLR" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="BLR_Point.xsd">
  <Versions>
    ...
    f.write('  <Classes>\n')
    f.write('    <Class name="BLR_Point">\n')
    f.write('      <Objects>\n')
    while(sh.cell(row=i,column=2).value!=None):
        c = str(sh.cell(row=i,column=2).value)[2:]
        f.write('        <Object name="SWB'+c+'" rules="update_or_create">\n')
        f.write('          <Properties>\n')
        f.write('            <MultiLingualProperty name="Name">\n')
        f.write('              <MultiLingualValue localeId="1033" roleId="-1">SWB'+c+'</MultiLingualValue>\n')
        f.write('              <MultiLingualValue localeId="1036" roleId="-1">SWB'+c+'</MultiLingualValue>\n')
