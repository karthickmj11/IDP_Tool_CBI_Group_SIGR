...
def run(loc,op_l):
    ...
    f = open(op_l+'\\ASCV.xml','w')
    sh = wb['Interlocking']
    f.write('''<?xml version="1.0" ?>
<ObjectPropertyModule Generation_Date="2019-02-25" Project="BLR" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="ASCV.xsd">
  <Versions>
...
''')
    f.write('  <Classes>\n')
    f.write('    <Class name="ASCV">\n')
    f.write('      <Objects>\n')
    f.write('        <Object name="'+str(sh.cell(row=1,column=2).value)+'" rules="update_or_create">\n')
    f.write('          <Properties>\n')
    f.write('            <MultiLingualProperty name="Name">\n')
    f.write('              <MultiLingualValue localeId="1033" roleId="-1">'+str(sh.cell(row=1,column=2).value)+'</MultiLingualValue>\n')
    f.write('              <MultiLingualValue localeId="1036" roleId="-1">'+str(sh.cell(row=1,column=2).value)+'</MultiLingualValue>\n')
